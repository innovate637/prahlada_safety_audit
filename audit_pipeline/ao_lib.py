...
def make_oracle_datapoint(
    ...
    # Two-step tokenization to guarantee a plain list[int].
    # In newer transformers, apply_chat_template(..., tokenize=True,
    # return_tensors=None) returns a BatchEncoding rather than a list, which
    # breaks len()/indexing semantics in find_pattern_in_tokens.
    ...
    input_ids = tokenizer(chat_str, add_special_tokens=False).input_ids
